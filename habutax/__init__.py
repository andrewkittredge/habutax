...
def prompt_input(missing, needed_by):
    ...
    for f in needed_by:
        ...
        instance = (
            f"Instance '{f.form().instance()}' of " if f.form().instance() else ""
        )
...
def solve(args):
    if args.writeback_input:
        Path(
            args.input_file
        ).touch()  # Ensure the input file exists (this allows writing back without the user having to manually touch it first)
...
def fill_pdfs(args):
    ...
    p = pdf_filler.PDFFiller(
        solution, forms.available_forms[tax_year], args.output, flatten=args.flatten
    )
    p.fill()
...
def list_forms(args):
    ...
    for form in forms.available_forms[args.year]:
        ...
        search_match = (
            ...
            or search_string in name.lower()
            or search_string in description.lower()
        )
        jurisdiction_match = (
            args.jurisdiction is None
            or args.jurisdiction.lower() == form.jurisdiction.name.lower()
        )
        if search_match and jurisdiction_match:
            if not header_printed:
                ...
                print(
                    format_str.format(
                        "name", "jurisdiction", "description", width=width
                    )
                )
                print(
                    format_str.format("-" * width, "-" * 12, "-" * width, width=width)
                )
            print(
                format_str.format(
                    name, form.jurisdiction.name, description, width=width
                )
            )
...
def list_form_inputs(args):
    ...
    if form_name not in forms_map:
        print(
            f'Cannot find form "{args.form}". The `list-forms` sub-command will list the valid form names.'
        )
        sys.exit(1)
    ...
    if hasattr(form_class, "valid_instances"):
        if form_instance not in form_class.valid_instances:
            ...
            print(
                f'Form "{form_name}" requires a form instance be specified and that it be one of {instances}. For example, try "{form_name}:{form_class.valid_instances[0]}".'
            )
            sys.exit(1)
...
def main():
    ...
    solve_parser.add_argument(
        "input_file",
        type=str,
        help="The file containing your input for the tax forms you are calculating.",
    )
    solve_parser.add_argument(
        "--year",
    ...
    solve_parser.add_argument(
        ...
        dest="forms",
        action="append",
        help="Which form(s) you want to calculate",
    )
    solve_parser.add_argument(
        "--prompt-missing",
        action="store_true",
        default=False,
        help="Interactively prompt for any missing input",
    )
    solve_parser.add_argument(
        "--writeback-input",
        action="store_true",
        default=False,
        help="Write any interactively-supplied input back to the config file when done (loses any comments/formatting present in file)",
    )
    solve_parser.add_argument(
        "--solution",
        type=str,
        default=None,
        help="Output text file for the results of the tax solver (defaults to stdout)",
    )
    solve_parser.set_defaults(func=solve)

    # fill-pdfs argument setup
    fill_pdfs_parser = subparsers.add_parser(
        "fill-pdfs",
        help="Fill PDFs using a solution previously calculated using HabuTax",
    )
    fill_pdfs_parser.add_argument(
        "--no-flatten",
        dest="flatten",
        action="store_false",
        default=True,
        help='Do not "flatten" the PDF after filling, so its fields can still be edited',
    )
    fill_pdfs_parser.add_argument(
        "solution",
        type=str,
        help="The file containing the solution of the tax forms you want to generate PDFs of.",
    )
    fill_pdfs_parser.add_argument(
        "output", type=str, help="Path where you want to write the generated PDF file"
    )
    fill_pdfs_parser.set_defaults(func=fill_pdfs)

    # list-forms argument setup
    list_forms_parser = subparsers.add_parser(
        "list-forms",
        help="List all available forms, optionally restricting to those containing a string or within a jurisdiction",
    )
    list_forms_parser.add_argument(
        "--year",
        type=int,
        default=default_year,
        help=f"The tax year to use (default: {default_year})",
    ...
    list_forms_parser.add_argument(
        ...
        type=str,
        default=None,
        help="A string to search for (only print forms containing this string)",
    )
    list_forms_parser.add_argument(
        "--jurisdiction",
        type=str,
        default=None,
        help='Only display forms from this tax jurisdiction ("US" federal, "NC" state, etc.)',
    )
    list_forms_parser.set_defaults(func=list_forms)

    # list-form-inputs argument setup
    list_form_inputs_parser = subparsers.add_parser(
        "list-form-inputs",
        help="List all available inputs for a given form, in a format suitable for editing and passing as input to HabuTax.",
    )
    list_form_inputs_parser.add_argument(
        "form",
        type=str,
        help="The short name of the form for which you want to display the inputs",
    )
    list_form_inputs_parser.add_argument(
        "--year",
        type=int,
    ...
    version_parser = subparsers.add_parser(
        "version", help="Display current HabuTax version."
    )
