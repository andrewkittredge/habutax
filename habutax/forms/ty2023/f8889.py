...
class Form8889(Form):
    ...
    def __init__(self, **kwargs):
        ...
        inputs = [
            BooleanInput(
                "part_2_needed",
                description=f"Do {you} need to fill out part 2 of Form 8999? Part 2 is used to report HSA distributions.",
            ),
            BooleanInput(
                "part_3_needed",
                description=f"Do {you} need to fill out part 3 of Form 8999? Part 3 is used to report cases when you fail to maintain high-deductible health plan (HDHP) coverage for the entire year.  See instructions.",
            ),
            BooleanInput(
                "hdhp_plan_family",
                description=f'Were you covered under a *family* high-deductible health plan (HDHP) during 2023 (as opposed to self-only coverage)? If {you} were covered, or considered covered, by a self-only high-deductible health plan (HDHP) and a family HDHP at different times during the year, answer "yes" only if the family plan was in effect for a longer period. If {you} were covered by both a self-only HDHP and a family HDHP at the same time, {you} are treated as having family coverage during that period. If, on the first day of the last month of {your} tax year (December 1 for most taxpayers), {you} had family coverage, answer "yes". Were you covered under a high-deductible *family* plan during 2023?',
            ),
            FloatInput(
                "hsa_contributions",
                description=f"Enter HSA contributions {you} made for 2023 (or those made on {your} behalf), including those made from January 1, 2023, through April 15, 2023, that were for 2023. Do not include employer contributions, contributions through a cafeteria plan, or rollovers. See instructions for Form 8889, line 2.",
            ),
            BooleanInput(
                "age_under_55",
                description=f"Were {you} under the age of 55 at the end of 2023?",
            ),
            BooleanInput(
                "hsa_full_year",
                description=f"Did {you} have a HDHP medical plan for the first day of every month during the 2023 year?",
            ),
            FloatInput(
                "archer_msa",
                description=f"Enter the amount {you} and {your} employer contributed to {your} Archer MSAs for 2023 from Form 8853, lines 1 and 2. If you or your spouse had family coverage under an HDHP at any time during 2023, also include any amount contributed to {your_spouses} Archer MSAs",
            ),
            BooleanInput(
                "qualified_distribution",
                description=f"Did {you} take a distribution from {your} traditional IRA or Roth IRA to {your} HSA in a direct trustee-to-trustee transfer?",
            ),
            FloatInput(
                "employer_contribution",
                description=f"Employer contributions made to {your} HSAs for 2023? NOTE: This number should match the sum of all {your} your forms W-2, box 12, code W.",
            ),
        ]
        ...
        def calc_contribution_max(self, i, v):
            if i["age_under_55"] and i["hsa_full_year"]:
                return (
                    self.threshold("hsa_family_contribution_limit")
                    if v["1"]
                    else self.threshold("hsa_individual_contribution_limit")
                )
            else:
        ...
        optional_fields = [
            ...
            FloatField(
                "4", lambda s, i, v: s.not_implemented() if i["archer_msa"] else 0.0
            ),
            ...
            FloatField(
                ...
                lambda s, i, v: (
                    s.not_implemented()
                    if i["1040_s1.hsa_contribution_you"]
                    and i["1040_s1.hsa_contribution_spouse"]
                    and i["hdhp_plan_family"]
                    else v["5"]
                ),
            ),
            FloatField(
                ...
                lambda s, i, v: (
                    s.not_implemented()
                    if not i["age_under_55"]
                    and (i["hdhp_plan_family"] or i["8889:spouse.hdhp_plan_family"])
                    else None
                ),
            ),
            ...
            FloatField(
                "10",
                lambda s, i, v: (
                    s.not_implemented() if i["qualified_distribution"] else None
                ),
            ),
            ...
            FloatField(
                ...
                lambda s, i, v: (
                    s.not_implemented()
                    if i["part_2_needed"] or i["part_3_needed"] or v["2"] > v["13"]
                    else v["13"]
                ),
            ),
        ]
