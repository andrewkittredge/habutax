...
class FormW2(InputForm):
    ...
    def __init__(self, **kwargs):
        ...
        for letter in "abcd":
            inputs.append(
                EnumInput(
                    f"box_12{letter}_code",
                    box_12,
                    allow_empty=True,
                    description=f"Box 12{letter} alphabetic code",
                )
            )
            inputs.append(
                FloatInput(f"box_12{letter}_value", description=f"Box 12{letter} value")
            )
        inputs += [
            EnumInput(
                "belongs_to",
                enum.taxpayer_or_spouse,
                description="To whom does this W-2 belong?",
            ),
            ...
            BooleanInput(
                "box_13_statutory", description="Box 13 statutory employee checked"
            ),
            BooleanInput(
                "box_13_retirement", description="Box 13 Retirement plan checked"
            ),
