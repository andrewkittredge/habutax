...
class Form1040S2Need6251(Form):
    ...
    def __init__(self, **kwargs):
        ...
        inputs = [
            BooleanInput(
                "schedule_j_needed",
                description="Do you need to fill our Schedule J? (Income Averaging for Farmers and Fishermen)",
            ),
            BooleanInput(
                "form_4972_needed",
                description="Do you need to fill out Form 4972? (Tax on Lump-Sum Distributions From Qualified Plans of Participants Born Before January 2, 1936)",
            ),
        ]
        ...
        def need_6251(self, i, v):
            if not v["5"] > v["6"]:
                return False
            elif v["11"] > self.threshold(
                "line_12_comparison", i["1040.filing_status"]
            ):
                return True
            return v["12"] > v["13"]
        ...
        optional_fields = [
            FloatField(
                "1",
                lambda s, i, v: (
                    s.not_implemented() if not v["1040.itemizing"] else v["1040.15"]
                ),
            ),
            FloatField(
                "2",
                lambda s, i, v: (
                    s.not_implemented() if not v["1040.itemizing"] else v["1040_sa.7"]
                ),
            ),
            FloatField(
                "3",
            ...
            FloatField(
                "4",
                lambda s, i, v: (
                    v["1040_s1.1"] + v["1040_s1.8z"]
                    if v["1040.schedule_1_additional_income"]
                    else None
                ),
            ),
            FloatField("5", lambda s, i, v: v["3"] - v["4"]),
            FloatField(
                "6", lambda s, i, v: s.threshold("line_6", i["1040.filing_status"])
            ),
            FloatField("7", lambda s, i, v: v["5"] - v["6"]),
            FloatField(
                "8", lambda s, i, v: s.threshold("line_8", i["1040.filing_status"])
            ),
            FloatField(
                "9", lambda s, i, v: 0.0 if v["5"] > v["8"] else v["5"] - v["8"]
            ),
            FloatField("10", lambda s, i, v: min(v["6"], v["9"] * 0.25)),
            FloatField(
                "11", lambda s, i, v: v["7"] + v["10"] if v["5"] > v["8"] else v["7"]
            ),
        ...
        super().__init__(
            __class__, inputs, [], optional_fields, thresholds=thresholds, **kwargs
        )
