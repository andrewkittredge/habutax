...
class Form1098(InputForm):
    ...
    def __init__(self, **kwargs):
        inputs = [
            EnumInput(
                "belongs_to",
                enum.taxpayer_spouse_or_both,
                description="To whom does this 1098 belong?",
            ),
            StringInput(
                "recipient_lender_name",
                description="Recipient/Lender's name: To whom did you pay this mortgage interest?",
            ),
            StringInput(
                "payer_borrower",
                description="Payer/Borrower: Name of borrower who paid the interest",
            ),
            FloatInput(
                "box_1",
                description="Mortgage interest received from payer(s)/borrower(s)",
            ),
            ...
            FloatInput(
                "box_6", description="Points paid on purchase of principal residence"
            ),
            BooleanInput("box_7", description="Is the checkbox on box 7 checked?"),
            StringInput(
                "box_8",
                description="Address or description of property securing mortgage",
            ),
            IntegerInput(
                "box_9", description="Number of properties securing the mortgage"
            ),
