...
class Form1040S8812(Form):
    ...
    def __init__(self, **kwargs):
        inputs = [
            BooleanInput(
                "pr_income_forms_2555_4563",
                description="Did you exclude any income from Puerto Rico, or require Forms 2555 or 2563?",
            ),
            IntegerInput(
                "number_under_18",
                description="How many of your dependents eligible for the child tax credit with the required social security number were under age 18 at the end of 2021?",
            ),
            IntegerInput(
                "number_under_6",
                description="How many of your dependents eligible for the child tax credit with the required social security number were under age 6 at the end of 2021?",
            ),
            BooleanInput(
                "principal_abode_us",
                description="Did you (or your spouse if married filing jointly) have a principal place of abode in the United States for more than half of 2021?",
            ),
            BooleanInput(
                "resident_puerto_rico",
                description="Were you (or your spouse of married filing jointly) a bona fide resident of Puerto Rico for 2021?",
            ),
            FloatInput(
                "advance_ctc_payments",
                description="Enter the aggregate amount of advance child tax credit payments you (and your spouse if filing jointly) received for 2021. See your Letter(s) 6419 for the amounts to include)",
            ),
            BooleanInput(
                "credit_limit_ws_credits",
                description="Are you using Schedule 3 to report any nonrefundable credits, Forms 5695, 8910, 8936, or Schedule R?",
            ),
            IntegerInput(
                "number_children_letter",
                description="Enter the number of qualifying children taken into account in determining the annual advance child tax credit amount you received for 2021. See your Letter 6419 for this number. If you are missing your Letter 6419, you are filing a joint return, or you received more than one Letter 6419, see the instructions before entering a number",
            ),
        ]
        ...
        optional_fields = [
            ...
            FloatField(
                "2a",
                lambda s, i, v: (
                    s.not_implemented() if i["pr_income_forms_2555_4563"] else None
                ),
            ),
            FloatField(
                "2b",
            ...
            FloatField(
                "14c", lambda s, i, v: 0.0 if v["14a"] < 0.001 else v["clwkst_a_5"]
            ),
            ...
            FloatField(
                "14h",
                lambda s, i, v: 0.0 if v["14g"] < 0.001 else min(v["14d"], v["14g"]),
            ),
            FloatField(
                "14i", lambda s, i, v: 0.0 if v["14g"] < 0.001 else v["14g"] - v["14h"]
            ),
            FloatField(
                "nonrefundable_ctc_or_odc",
                lambda s, i, v: (
                    v["14h"] if v["13a"] or v["13b"] else s.not_implemented()
                ),
            ),  # -> Form 1040, line 19
            FloatField(
                "refundable_ctc_or_additional_ctc",
                lambda s, i, v: (
                    v["14i"] if v["13a"] or v["13b"] else s.not_implemented()
                ),
            ),  # -> Form 1040, line 28
            # Omitting Parts I-C, II-A, II-B, II-C - they are only used if you did *not* check a box on line 13
            FloatField(
                "28a", lambda s, i, v: v["14f"] if v["13a"] or v["13b"] else v["15e"]
            ),
            FloatField(
                "28b", lambda s, i, v: v["14e"] if v["13a"] or v["13b"] else v["15d"]
            ),
            ...
            FloatField(
                "36", lambda s, i, v: ceil((v["34"] / v["35"]) / 1000.0) * 1000.0
            ),
        ...
        optional_fields += [
            ...
            FloatField(
                "clwkst_a_2",
                lambda s, i, v: (
                    s.not_implemented() if i["credit_limit_ws_credits"] else None
                ),
            ),
            FloatField(
                "clwkst_a_3",
                lambda s, i, v: (
                    s.not_implemented()
                    if not v["13a"] and not v["13b"]
                    else v["clwkst_a_1"] - v["clwkst_a_2"]
                ),
            ),
            FloatField(
                "clwkst_a_4",
                lambda s, i, v: (
                    s.not_implemented() if not v["13a"] and not v["13b"] else 0.0
                ),
            ),
        ...
        required_fields = [
            BooleanField(
                ...
                lambda s, i, v: (
                    ...
                    == v["4a"]
                    else s.not_implemented()
                ),
            ),  # TODO would be more user-friendly to be reported as some type of error rather than "not implemented"
        ]

        pdf_fields = [
            TextPDFField("topmostSubform[0].Page1[0].f1_01[0]", "1040.full_names"),
            TextPDFField(
                "topmostSubform[0].Page1[0].f1_02[0]", "1040.you_ssn", max_length=11
            ),
            ...
            ButtonPDFField(
                "topmostSubform[0].Page1[0].Line13_ReadOrder[0].c1_1[0]", "13a", "1"
            ),
            ButtonPDFField(
                "topmostSubform[0].Page1[0].Line13_ReadOrder[0].c1_2[0]", "13b", "1"
            ),
