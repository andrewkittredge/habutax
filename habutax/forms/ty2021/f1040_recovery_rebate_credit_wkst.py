...
class Form1040RecoveryRebateCreditWkst(Form):
    ...
    def __init__(self, **kwargs):
        inputs = [
            BooleanInput(
                "ssn_before_due_date",
                description="Does your 2021 return include a social security number that was issued on or before the due date of your 2021 return (including extensions) for you and, if filing a joint return, your spouse?",
            ),
            BooleanInput(
                "armed_forces",
                description="Was at least one of you or your spouse a member of the U.S. Armed Forces at any time during 2021, and does at least one of you have a social security number that was issued on or before the due date of your 2021 return (including extensions)?",
            ),
            BooleanInput(
                "either_ssn_before_due_date",
                description="Does one of you or your spouse have a social security number that was issued on or before the due date of your 2021 return (including extensions)?",
            ),
            IntegerInput(
        ...
        def line_6(self, i, v):
            amount = 1400.0
            if i["1040.filing_status"] is filing_status.MarriedFilingJointly and (
                v["2"] or v["3"]
            ):
                amount = 2800.0
            if not v["2"]:
        ...
        optional_fields = [
            BooleanField(
                ...
                lambda s, i, v: (
                    False
                    if i["1040.filing_status"] is filing_status.MarriedFilingJointly
                    else i["1040.claimed_as_dependent"]
                ),
            ),
            ...
            FloatField("6", line_6),
            FloatField(
                "7", lambda s, i, v: 1400.0 * i["dependents_ssn_before_due_date"]
            ),
            ...
            FloatField(
                ...
                lambda s, i, v: (
                    s.not_implemented()
                    if v["9"] > line_10_amount(s, i, v)
                    else line_10_amount(s, i, v) - v["9"]
                ),
            ),
            FloatField("11", line_11),
            FloatField(
                "12", lambda s, i, v: (v["8"] * v["11"]) if v["9_checkbox"] else v["8"]
            ),
            FloatField("13", lambda s, i, v: i["eip_3_amount"]),
            FloatField("14", lambda s, i, v: max(0.0, v["12"] - v["13"])),
            FloatField(
                "credit", lambda s, i, v: v["14"] if can_take_credit(s, i, v) else None
            ),
        ]
